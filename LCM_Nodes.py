...
class LCMGenerate:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "prompt_weighting":(["enable","disable"],)}
        }
    ...
    def mainfunc(self, text: str,steps: int,width:int,height:int,gfc:float,seed: int,image,mask,original_image,outpaint_size,outpaint_direction,mode,pipe,batch,prompt_weighting):
        '''
        # Save Path:
        save_path = "./lcm_images"
        ...
        if prompt_weighting == "enable":
            ...
            # Output Images:
                ...
                images = pipe(prompt_embeds=prompt_embeds, num_images_per_prompt=1, num_inference_steps=steps, guidance_scale=gfc, lcm_origin_steps=50,width=width,height=height,strength = 1.0, image=image, mask_image=mask).images
        else:
            for i in range(0,batch):
                seed = random.randint(0,1000000000000000)
                torch.manual_seed(seed)
...
class LCMGenerate_img2img:
    ...
    def mainfunc(self, text: str,steps: int,width:int,height:int,gfc:float,seed: int,image,outpaint_size,outpaint_direction,mode,pipe,batch,strength,prompt_weighting):
        '''
        # Save Path:
        save_path = "./lcm_images"
        ...
        if prompt_weighting == "enable":
            ...
            # Output Images:
                ...
                images = pipe(prompt_embeds=prompt_embeds, num_images_per_prompt=1, num_inference_steps=steps, guidance_scale=gfc, lcm_origin_steps=50,width=width,height=height,strength = strength, image=image).images
                res.append(images[0])
        else:
            for i in range(0,batch):
                seed = random.randint(0,1000000000000000)
            ...
            # Output Images:
                ...
                images = pipe(prompt=prompt, num_images_per_prompt=1, num_inference_steps=steps, guidance_scale=gfc, lcm_origin_steps=50,width=width,height=height,strength = strength, image=image).images
                res.append(images[0])
