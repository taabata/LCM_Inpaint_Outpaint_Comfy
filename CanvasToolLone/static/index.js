...
function generate(){
    fetch('http://localhost:5000/generate', {
        method: 'POST',
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({"wf":selwf,"model":selmod})
    })
    .then(function (response){
        return response.json();
    })
...
function saved(savedata){
    ...
    fetch('http://localhost:5000/savedata', {
        method: 'POST',
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({"savedata":savedata,"selectorsize":selectorsize,"taesd":taesd})
    })
    .then(function (response) {
        responseClone = response.clone(); // 2
        return response.json();
    ...
    .then(data => {
        ...
        if(data["taesd"]=="no"){
            if(canvasflag){
                ...
                document.getElementById("source").width = data["width"];
                document.getElementById("source").height = data["height"];
                /*document.getElementById("sourceunder").width = data["width"];
                document.getElementById("sourceunder").height = data["height"];*/
            ...
            //document.getElementById("sourceunder").style.visibility = "visible";        
            

            /*document.getElementById("sourceunder").style.left = "0px";
            document.getElementById("sourceunder").style.top = document.getElementById("source").style.top;*/
        ...
        /*setTimeout(() => {
            var canvasunder = document.getElementById("sourceunder");
            var ctxunder = canvasunder.getContext("2d");
            ctxunder.clearRect(0, 0, canvasunder.width, canvasunder.height);
            ctxunder.drawImage(img, 0, 0);
        }, 100);*/
