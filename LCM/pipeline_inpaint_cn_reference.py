...
from diffusers import AutoencoderKL, ConfigMixin, DiffusionPipeline, SchedulerMixin, UNet2DConditionModel, logging, AsymmetricAutoencoderKL, ControlNetModel
...
def torch_dfs(model: torch.nn.Module):
    ...
    return result
class LatentConsistencyModelPipeline_refinpaintcn(DiffusionPipeline):
    ...
    def prepare_ref_latents(self, refimage, batch_size,num_channels_latents,height,width, dtype, device, generator=None):
        shape = (batch_size, num_channels_latents, height // self.vae_scale_factor, width // self.vae_scale_factor)
        if not isinstance(refimage, (torch.Tensor, PIL.Image.Image, list)):
            raise ValueError(
                f"`image` has to be of type `torch.Tensor`, `PIL.Image.Image` or list but is {type(image)}"
            )

        refimage = refimage.to(device=device, dtype=dtype)
        if refimage.shape[1] == 4:
            ref_image_latents = refimage
            ...
            if isinstance(generator, list) and len(generator) != batch_size:
                ...
                ref_image_latents = [
                    self.vae.encode(refimage[i : i + 1]).latent_dist.sample(generator[i]) for i in range(batch_size)
                ]
                ref_image_latents = torch.cat(ref_image_latents, dim=0)
            else:
                ref_image_latents = self.vae.encode(refimage).latent_dist.sample(generator)
    ...
    def prepare_latents(
        self, image, timestep, batch_size, num_channels_latents, height, width, dtype, device, latents=None, generator=None,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if guess_mode:
                    ...
                    control_model_input = self.scheduler.scale_model_input(control_model_input, ts)
